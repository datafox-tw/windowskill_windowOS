...
bool LTexture::loadFromRenderedText(TTF_Font* Font, std::string textureText, SDL_Color textColor, SDL_Renderer* renderer ){
	free();	//Get rid of preexisting texture

	SDL_Surface* textSurface = TTF_RenderText_Solid( Font, textureText.c_str(), textColor );	//Render text surface
	if( textSurface != NULL ){
...
bool loadMedia(){
	...
	gFont28 = TTF_OpenFont( "img/FUT_R.ttf", 28 );	//Open the font
	gFont36 = TTF_OpenFont( "img/FUT_R.ttf", 36 );
	gFont16 = TTF_OpenFont( "img/FUT_R.ttf", 16 );
	if( gFont28 == NULL || gFont36 == NULL || gFont16 == NULL){
